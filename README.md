...
- `crates/oxide-torch`: Tensor、autograd、`nn`、optimizer、データ、checkpoint、JIT、backend adapter
- `crates/oxide-torch-cuda`: cuda-oxide kernel、CUDA Graph、cuBLAS、cuDNN
- `crates/oxide-torch-models`: Gemma4、MobileNetV4、およびモデル専用CUDA実行コード

自作モデルや学習基盤だけが必要なら `oxide-torch` のみに依存できます。標準モデルを
利用する場合だけ `oxide-torch-models` を追加してください。依存方向は
modelsはcoreへ依存し、CUDA feature時だけcoreとmodelsが `oxide-torch-cuda` の
公開backend APIへ依存します。CUDA featureを無効にした場合、CUDA crateの外部
CUDA依存は有効化されません。
...
cargo +stable run -p oxide-torch --example mlp
```
...
```bash
CUDA_OXIDE_TARGET=sm_80 cargo oxide build --arch sm_80 -p oxide-torch-models --features cuda -- \

...
cargo oxide doctor
cargo oxide run mnist-training --features cuda --bin mnist-training
```
...
今後の拡張点です。BatchNorm2dは学習可能なγ/β、running mean/variance、train/eval
切替、autogradに対応します。CUDAのBatchNorm forwardとbackwardは正しさを優先した
CPU fallbackです。
...
`mobilenetv4-mnist.oxtr` を保存します。checkpointには学習parameterに加えてBatchNormの
running mean/varianceも含まれます。

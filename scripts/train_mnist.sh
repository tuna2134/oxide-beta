...
if ((USE_CUDA == 1)); then
    ...
    exec cargo oxide run -p oxide-torch-models --features cuda,cudnn --bin mnist-training
else
    exec cargo +stable run -p oxide-torch-models --release --example mnist_training -- "${DATA_DIR}"
fi

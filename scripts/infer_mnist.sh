...
if [[ "${1:-}" == "--cuda" ]]; then
    ...
    exec cargo oxide run -p oxide-torch-models --features cuda,cudnn --bin mnist-inference
fi
exec cargo +stable run -p oxide-torch-models --release --example mnist_inference
